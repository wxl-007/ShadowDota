...
local Common       = require("NetInterFace.hprose.common")
local Tags         = require("NetInterFace.hprose.tags")
local ClassManager = require("NetInterFace.hprose.class_manager")
...
function Writer:writeDateTime(val)
    ...
    if isdate(val) then
        ...
        if istime(val) then
            ...
            self.stream:write(Tags.Date, osdate("%Y%m%d", time), Tags.Time, osdate("%H%M%S", time), timezone)
        end
    end
end
...
function Writer:writeClass(classname, fields)
    ...
    if len < 0 then 
        error("class name must be encoding in utf8.") 
    end
