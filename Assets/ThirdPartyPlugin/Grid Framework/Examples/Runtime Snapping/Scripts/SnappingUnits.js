...
*/
public var grid : GFGrid; // the grid we want to snap to
private var gridCollider : Collider ; // a collider attached to the grid, will be used for handling muse input
	
private var beingDragged : boolean; // true while the player is dragging the block around, otherwise false
private var oldPosition : Vector3; // the previous valid position

private var cachedTransform : Transform; //cache the transform for performance
...
function DragObject(){
	...
	var cursorWorldPoint : Vector3 = ShootRay();
...
function ShootRay() {
	var hit : RaycastHit;
	gridCollider.Raycast(Camera.main.ScreenPointToRay (Input.mousePosition), hit, Mathf.Infinity);
...
private var _intersecting : int = 0;
...
public function SetIntersecting(intersecting : boolean){
	...
	function TintRed(intersections : int){
	if(intersections > 0){
...
private function SetupRigidbody(){
	var rb : Rigidbody = GetComponent.<Rigidbody>(); //the rigidbody component of this object
...
private function ConstructTrigger(){
