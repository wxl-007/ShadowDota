...
public class AIPath : MonoBehaviour {
	 ...
	 */
	public const float repathRate = 0.5F;
...
//	public Transform target;
    ...
    private readonly Vector3 INIT_DEST = new Vector3(Mathf.Infinity, Mathf.Infinity, Mathf.Infinity);
    private Vector3 mDestination = new Vector3(Mathf.Infinity, Mathf.Infinity, Mathf.Infinity);
...
//		if (Time.time - lastRepath >= repathRate && canSearchAgain && canSearch && target != null) {
        if (Time.time - lastRepath >= repathRate && canSearchAgain && canSearch && mDestination != INIT_DEST) {
			SearchPath ();
			return repathRate;
		} else {
...
//		if (target == null) throw new System.InvalidOperationException ("Target is null");
        if (mDestination == INIT_DEST) throw new System.InvalidOperationException ("Target is null");

		lastRepath = Time.time;
		//This is where we should search to
//		Vector3 targetPosition = target.position;
        Vector3 targetPosition = mDestination;
