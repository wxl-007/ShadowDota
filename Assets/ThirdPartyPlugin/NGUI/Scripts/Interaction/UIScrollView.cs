...
public class UIScrollView : MonoBehaviour
{
	...
	public virtual void MoveRelative (Vector3 relative)
	{
        ...
        Debug.Log(" clip offset  == (" + co.x+","+ co.y +") " + " to move "  );
	...
	public void Press (bool pressed)
	{
		...
		if (enabled && NGUITools.GetActive(gameObject))
		{
			...
			{
                if (centerOnChildForMain != null)
                {

                    if (centerOnChildForMain != null)
                    {
                        centerOnChildForMain.Recenter();
                    }
                    else
                    {
    ...
    //wxl 
    [HideInInspector]
    public CenterOnChildForMain centerOnChildForMain = null;
	/// <summary>
		...
		if (!mPressed)
		{
			if (mMomentum.magnitude > 0.0001f || mScroll != 0f)
			{
				...
				if (restrictWithinPanel && mPanel.clipping != UIDrawCall.Clipping.None)
				{
                    if (centerOnChildForMain != null)
                    {
                        if (NGUITools.GetActive(centerOnChildForMain))
                        {
                            if (centerOnChildForMain.nextPageThreshold != 0f)
                            {
                                mMomentum = Vector3.zero;
                                mScroll = 0f;
                            }
    ...
    #region    时时  计算 rotation  和  scale               coding by  wxl  

    //  最终移动 方法 
    public void MoveAbsoluteForMain (Vector3 absolute)
    {
        Vector3 a = mTrans.InverseTransformPoint(absolute);
        Vector3 b = mTrans.InverseTransformPoint(Vector3.zero);
        MoveRelativeForMain(a-b);
    }

    public void MoveRelativeForMain(Vector3 relative){
//        mTrans.localPosition += relative;
        Vector2 co = mPanel.clipOffset;
        co.x -= relative.x;
        co.y -= relative.y;
//        mPanel.clipOffset = co;
//        Debug.Log(" clip offset  == (" + co.x+","+ co.y +") " + " to move "  + relative );
        centerOnChildForMain.OnCalculate(relative);
...
//            Debug.Log(" in drag for main " + smoothDragStart   +   "  msatart  " + mDragStarted + " mPlane.Raycast  == " + mPlane);
            if (mPlane.Raycast(ray, out dist))
            {
                Vector3 currentPos = ray.GetPoint(dist);
                Vector3 offset = currentPos - mLastPos;
...
//                Debug.Log( "  offset "  + offset  + " cur pos =    " +currentPos + " last pos = "+   mLastPos );
                // Adjust the momentum
                if (dragEffect == DragEffect.None) mMomentum = Vector3.zero;
                else mMomentum = Vector3.Lerp(mMomentum, mMomentum + offset * (0.01f * momentumAmount), 0.67f);
                // Move the scroll view
                ...
                {
//                    MoveAbsolute(offset);
//                    MoveAbsoluteForMain(offset);
                    MoveAbsoluteForMain(new Vector3(currentPos.x,0,0));
    ...
    public void OnPressForMain(bool pressed){
        if (UICamera.currentScheme == UICamera.ControlScheme.Controller) return;
