...
//                scrollView.OnDragForMain();
    ...
    //取消 拖拽  非居中 时候 
    void CancelDrag(){
        if(scrollView && scrollView.centerOnChildForMain ){
            if (transform.gameObject == scrollView.centerOnChildForMain.centeredObject)
            {
                scrollView.OnDragForMain();
            }
        }
    ...
    void CancelPress(bool press ){
        if(scrollView && scrollView.centerOnChildForMain ){
            if (transform.gameObject == scrollView.centerOnChildForMain.centeredObject)
            {
                scrollView.OnPressForMain(press);
            }
//            else
//            {
//                scrollView.centerOnChildForMain.CenterOn(gameObject.transform);
//            }
        }
    }
