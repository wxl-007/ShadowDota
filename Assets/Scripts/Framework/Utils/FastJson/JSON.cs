...
namespace fastJSON
{
    ...
    public sealed class JSONParameters
    {
		...
		/// 转换为string的时候不使用转义字符
		/// </summary>
		public bool UseEscapedCharactor = true;
    ...
    public sealed class JSON
    {
        ...
        private myPropInfo CreateMyProp(Type t, string name)
        {
			...
			else if (t == typeof(short) || t == typeof(short?)) d_type = myPropInfoType.Short;
			else if (t == typeof(float) || t == typeof(float?)) d_type = myPropInfoType.Float;
            else if (t.IsArray)
            {
                ...
                if ((pi.Flags & (myPropInfoFlags.Filled | myPropInfoFlags.CanWrite)) != 0)
                {
                    ...
                    if (v != null)
                    {
                        ...
                        switch (pi.Type)
                        {
							...
							case myPropInfoType.Enum: oset = CreateEnum(pi.pt, v.ToString()); break;
        ...
        private object CreateGenericList(List<object> data, Type pt, Type bt, Dictionary<string, object> globalTypes)
        {
            ...
            {
            	Type type = Type.GetType("System.Collections.Generic.List`1[" + bt.FullName + "]");
            	col = (IList)Reflection.Instance.FastCreateInstance(type);
            }
            ...
            foreach (object ob in data)
            {
                if (ob is IDictionary)
					...
					List<object> mob = (List<object>)ob;
					int count = mob.Count;

					if(col is List<int[]>) {
						int[] tmp = new int[count];
						for(int i = 0; i < count ; ++ i)
							tmp[i] = Convert.ToInt32 (mob[i]);
						col.Add(tmp);
					} else if(col is List<short[]>) {
						short[] tmp = new short[count];
						for(int i = 0; i < count ; ++ i)
							tmp[i] = Convert.ToInt16 (mob[i]);
						col.Add(tmp);
					} else if(col is List<long[]>) {
						long[] tmp = new long[count];
						for(int i = 0; i < count ; ++ i)
							tmp[i] = Convert.ToInt64 (mob[i]);
						col.Add(tmp);
					} else if(col is List<string[]>) {
						string[] tmp = new string[count];
						for(int i = 0; i < count ; ++ i)
							tmp[i] = Convert.ToString(mob[i]);
						col.Add(tmp);
					} else if(col is List<float[]>) {
						float[] tmp = new float[count];
						for(int i = 0; i < count ; ++ i)
							tmp[i] = Convert.ToSingle (mob[i]);
						col.Add(tmp);
					} else {
					...
					Type[] genericArgType = pt.GetGenericArguments();
					if(genericArgType != null && genericArgType.Length > 0) {
						col.Add(ChangeType(ob, genericArgType[0]));
					}
				}
