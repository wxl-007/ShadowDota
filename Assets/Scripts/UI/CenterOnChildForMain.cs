...
public class CenterOnChildForMain : MonoBehaviour
{
    ...
    //主界面3d拖动 部分
    public Main3DUIScrollView mainSV;
    ...
    void OnEnable()
    {
        if (mScrollView)
        {
            mScrollView.centerOnChildForMain = this;
            Recenter();
        }
    }
    ...
    {
        if (mScrollView)
            mScrollView.centerOnChildForMain = null;
    }
...
//            CircleRender();
    ...
    void CenterOn(Transform target, Vector3 panelCenter)
    {
        ...
        {
            ResetOtherOn(target);
            mainSV.ShowCenterEffect(target);    
            mScrollView.panel.clipOffset = Vector2.zero;
            mScrollView.panel.GetComponent<SpringPanel>().target = Vector3.zero;
            mScrollView.GetComponent<SpringPanel>().enabled =true;
        } 
    }
    ...
    public void ResetOtherOn(Transform target)
    {
        UIGrid grid = GetComponent<UIGrid>();
        if (grid != null && mainSV != null)
        {
            List<Transform> list = grid.GetChildList();
            int tIndex = list.IndexOf(target);
		
            for (int i = 0; i < list.Count; i++)
            {
                if (tIndex > i)
                {
                    mainSV.OtherTransform(list[i], -1, tIndex);
                }
                else if (tIndex < i)
                {
                    mainSV.OtherTransform(list[i], 1, tIndex);
    ...
    public void OnCalculate( Vector3 move ){
		if (mScrollView != null /*&& mScrollView.centerOnChildForMain != null */)
        {
            if (mScrollView.isDragging)
            {
                mainSV.OnUpdateCalculate(move);
            }
        }
    }

    public void CenterOnForMain(Transform target){
        if (mScrollView != null && mScrollView.panel != null)
        {
